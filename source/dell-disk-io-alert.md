监测平台报出 IO 占用率和延迟过高的告警，带外却查不到任何异常日志时，排障就得从操作系统内部继续。本文按 JBOD 直通和 RAID 阵列两种模式分别整理排查思路，附上可直接复制粘贴的批量巡检脚本，并对 `smartctl` 的原始错误日志逐项解读。

## 系统报 IO 告警，在带外无异常的情况下，在操作系统中进行排障
...
范例中 ECC 纠错 2555 次，且存在 4 次无法纠错，且非介质故障高达 13 次，该硬盘急需维护。
