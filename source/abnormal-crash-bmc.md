服务器异常宕机，带外日志里却只有一条热复位记录，既没有物理按键记录，也没有操作系统发起的重启记录，这种时候该从哪里下手？本文整理一套在 BMC 日志无参考价值时的排障思路与对应命令。
...
- **排查 watchdog 是否触发**：`journalctl -k | grep -i "watchdog"` 和 `dmesg | grep -i "watchdog"`
