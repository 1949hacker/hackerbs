本文记录了我把一个用 pyinstaller 打包出来的二进制可执行程序，通过 `dh_make` 和 `dpkg-buildpackage` 构建成标准 deb 包的全过程。适合需要把自研命令行工具分发到多台 Debian 主机、又不想手动拷贝二进制的场景。

构建 deb 包所需依赖：

`apt install dh-make`

本文使用本站 aptdownloader 包做为演示。
...
至此，构建 deb 包的教程结束，请移步《如何搭建具有 GPG key 验证的可信任 apt 源》。
